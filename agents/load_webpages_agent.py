...
class LoadWebPageAgent:
    """Encapsulates MCP Load Web Page Agent lifecycle and interactions."""

    def __init__(self, server_script_path: str):
        """
        Initialize the agent with the path to the MCP server script.

        :param server_script_path: Absolute path to the MCP server script file.
        """
    ...
    async def start(self):
        ...
        self.agent = LlmAgent(
            ...
            instruction="Help the user load web pages using the available tools.",
            tools=tools,
        )
    ...
    async def ask(self, user_input: str) -> str:
        """Send user input to the agent and return the response text."""
    ...
    async def shutdown(self):
        """Graceful shutdown of MCP Toolset."""
        if self.toolset:
            try:
    ...
    def _extract_text_from_event(self, event) -> str:
        """Helper to extract readable text or function information from streamed events."""
        texts = []
